...
// TURN_APP_NAME es el "Metered Domain" completo que te muestra el dashboard (ej: ytjam.metered.live), NO solo el nombre.
  ...
  if (req.method === 'GET' && req.url === '/turn-credentials') {
    ...
    try {
      const upstream = await fetch(
        `https://${TURN_APP_NAME}/api/v1/turn/credentials?apiKey=${TURN_API_KEY}`
      );
